...
## Advanced features backlog (Power upgrade)

To make Veronica significantly more powerful, here is the next advanced feature set to implement:

1. Plugin marketplace and dynamic skill loader (`plugins/` manifests + hot reload).
2. Long-term memory v2 (profile memory, workspace memory, editable memory entries).
3. Local RAG over files (PDF/MD/TXT indexing + citation answers).
4. Agentic task runner (multi-step plans, retries, checkpoints, background jobs).
5. Advanced offline STT modes (wake word, push-to-talk, model selection).
6. Desktop automation macros (record/replay workflows with safety checks).
7. Security shield v2 (zones, schedules, escalation rules, event timeline).
8. Mobile upgrade (push notifications, richer controls, optional native wrapper).
9. Proactive briefings (morning summary, reminders digest, productivity nudges).
10. Workspace modes (`work`, `study`, `silent`, `travel`) with behavior presets.
11. Policy/safety engine for risky commands (dry-run + approval levels).
12. Observability pack (structured logs, latency metrics, diagnostics command).
13. Expanded test harness (race-condition tests, plugin compatibility suites).
14. Optional encrypted multi-device sync for notes/reminders/preferences.
15. Optional local LLM adapter mode while keeping deterministic local AI default.

Suggested release slicing:
- **v3.1**: memory v2 + diagnostics + STT upgrade.
- **v3.2**: local RAG + plugin architecture.
- **v3.3**: agentic tasks + automation macros.
- **v4.0**: proactive assistant + mobile push + policy engine.


## Optional Gemini API mode

By default Veronica uses its deterministic fully local AI backend. If you want Gemini responses for general chat, enable it explicitly:

```bash
export VERONICA_AI_PROVIDER="gemini"
...
Provider options:
- `VERONICA_AI_PROVIDER=local` (default deterministic local backend)
- `VERONICA_AI_PROVIDER=gemini` (hosted Gemini responses for general chat)

Backward compatibility: `VERONICA_USE_GEMINI=1` still works, but `VERONICA_AI_PROVIDER=gemini` is recommended.


## Advanced shortcuts (macro-like aliases)

You can define command aliases in `shortcuts.json` inside your Veronica data directory (default `~/.veronica/shortcuts.json`).

Example:

```json
{
  "morning": "weather in Mumbai and then show reminders",
  "brief": "what is today's date"
}
```
...
Supported trigger prefixes are `shortcut`, `run`, and `do`. This gives you lightweight automation macros without changing code.


## Advanced roadmap tracker (step-by-step execution)

Veronica now includes a built-in tracker so you can implement advanced upgrades one-by-one without missing any feature.

Commands:

- `advance status` → show all advanced steps and completion state
- `advance next` → show the next pending step
- `advance done <step_key>` → mark one step complete
- `advance reset` → reset roadmap progress

Progress is stored in `advanced_progress.json` inside your data directory.


## OpenClaw-style bridge

Veronica includes two OpenClaw paths: a lightweight OpenClaw-inspired bridge for Python-native tools, plus a full-runtime adapter that can call the real `openclaw` CLI when you install it with npm or point Veronica at a source checkout.
...
openclaw status
openclaw tools
openclaw run roadmap_next
openclaw run roadmap_status
openclaw run memory_list
openclaw run bridge_complete
openclaw full status
openclaw full install
openclaw full doctor
openclaw full gateway status
openclaw full agent Ship checklist
openclaw full raw --help
```

Optional custom tool descriptors can be placed in `openclaw_tools.json`, `openclaw_manifest.json`, or `openclaw/tools.json` inside the assistant data directory. Example:

```json
{
  "tools": [
    {"name": "brief", "description": "Show next roadmap step", "kind": "assistant", "command": "advance next"},
    {"name": "safe_echo", "description": "Trusted local check", "kind": "shell", "command": "python -c \"print('ok')\"", "requires_approval": false}
  ]
}
```

Assistant-mapped tools are listed safely. Shell tools are denied by default and run only when a trusted manifest sets `requires_approval` to `false`; keep that setting only for commands you personally trust. For the full OpenClaw runtime, install `openclaw` globally (`npm install -g openclaw@latest`) or set `VERONICA_OPENCLAW_SOURCE=/path/to/openclaw` for a source checkout; Veronica then forwards `openclaw full ...` commands to the real runtime instead of the bridge.
