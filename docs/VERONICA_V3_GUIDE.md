...
| Offline AI | Built-in deterministic local backend by default | No hosted model needed unless Gemini mode is enabled |
...
- Plugin marketplace with hot-loaded skills and permission boundaries.
- Long-term memory v2 with user/workspace context controls.
- Offline local RAG over personal docs with cited answers.
- Agentic background task runner with plan steps and retries.
- Advanced offline STT and wake-word control.
- Desktop macro record/replay automation workflows.
- Security v2 with schedules, zones, and escalation policies.
- Mobile push notifications and richer command panel actions.
- Proactive briefing engine (daily summaries and nudges).
- Workspace behavior modes and safety policy profiles.
- Structured telemetry and built-in diagnostics toolkit.
- Optional encrypted device sync and optional local LLM adapter mode.


### Optional Gemini mode

```bat
set VERONICA_AI_PROVIDER=gemini
set GEMINI_API_KEY=your-gemini-key
set GEMINI_MODEL=gemini-1.5-flash
python -m veronica
```

Gemini mode is opt-in. By default `VERONICA_AI_PROVIDER=local`, so Veronica stays on the local deterministic backend. Legacy flag `VERONICA_USE_GEMINI=1` also works.
